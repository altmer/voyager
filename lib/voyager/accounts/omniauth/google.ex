defmodule Voyager.Accounts.Omniauth.Google do
  ...
  defp register_user_if_needed(user, _) when user != nil, do: {:ok, user}

  defp generate_token({:error, _} = error_tuple), do: error_tuple
  defp generate_token({:ok, user}), do: Sessions.gen_token(user)
end
